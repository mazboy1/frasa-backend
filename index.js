  ...
  async function run() {
    ...
    app.get('/api/cart-item/:classId', async (req, res) => {
      try {
        ...
        console.log('🔍 [GET /api/cart-item] Checking if class in cart:', {
          classId,
        ...
        // ✅ FIX: Try both string dan ObjectId
        ...
        if (!cartItem) {
          try {
            cartItem = await cartCollection.findOne({
              classId: new ObjectId(classId),
              userMail: email
            });
          } catch (e) {
            // Ignore ObjectId conversion error
          }
        }
        ...
        if (cartItem) {
          ...
          console.log('📊 Debug: Looking for classId:', classId);
          res.status(404).send({
            success: false,
            classId: classId,
